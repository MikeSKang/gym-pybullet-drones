...
class SiameseNet(nn.Module):
    ...
    def __init__(self, embedding_net, upscale=False, corr_map_size=33, stride=4):
        ...
        #self.match_batchnorm = nn.Identity()
    ...
    def match_corr(self, embed_ref, embed_srch):
        ...
        """
        eps = 1e-6
        embed_ref  = embed_ref  / (embed_ref.norm(p=2, dim=1, keepdim=True)  + eps)
        embed_srch = embed_srch / (embed_srch.norm(p=2, dim=1, keepdim=True) + eps)
